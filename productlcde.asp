...
id=GetParaNum("id")	
	...
	set rs=Server.CreateObject("ADODB.Recordset")
		...
		if request.Form("butmod")<>"" then
			...
			rs.update()
			rs.close()
			response.Redirect("productlcde.asp?id="& id )
		end if
	
	sql="select * from sq_jinrongcity.[Lc_tab] where id="& id
	if rs.state=1 then rs.close()
	rs.open sql,conn,1,3	
	if not rs.eof then 
...
<div class="content3">
  ...
  <!--<strong style="color:#F30">主推产品：</strong>%=titleStr%>
  <DIV style="HEIGHT:10px"></DIV>
  <strong style="color:#F30">起购金额：</strong>%=moneyStr%>
  <DIV style="HEIGHT:10px"></DIV>
  <strong style="color:#F30">年&nbsp;&nbsp;利&nbsp;率：</strong>%=yearStr%>
  <DIV style="HEIGHT:10px"></DIV>
  <strong style="color:#F30">期&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;限：</strong>%=bigStr%>
  <DIV style="HEIGHT:10px"></DIV>
  <strong style="color:#F30">进&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;度：</strong>%=jinduStr%>
  <DIV style="HEIGHT:10px"></DIV>
  <strong style="color:#F30">星&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;级：</strong>%=xingjiStr%>
  <DIV style="HEIGHT:10px"></DIV>-->
  <%
