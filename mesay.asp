...
<%
	...
	set rs=Server.CreateObject("ADODB.Recordset")
		...
		sql = "select id,content,xingji,product,dianpin from ("
		
		sql = sql & " select a.id,content,xingji,product,a.dianpin from UserPl_tab a inner join sq_jinrongcity.[Lc_tab] b on a.FNewID=b.id where a.FType=1 and a.FUserID="& session("memeber_Id") 
		
		sql = sql & " union all "
		
		sql = sql & " select a.id,content,xingji,product,a.dianpin from UserPl_tab a inner join sq_jinrongcity.[Bx_tab] b on a.FNewID=b.id where  a.FType=2 and a.FUserID="& session("memeber_Id") 
...
<div class="content3">
  ...
  <DIV style="HEIGHT:10px"></DIV>
  <strong style="color:#C60">点评内容：</strong><%=rs("dianpin")%>
  <DIV style="HEIGHT:10px"></DIV>
  <a href="mesay_del.asp?id=<%=rs("id")%>" target="_parent"><strong style="color:#F00">删除点评</strong></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="mesay_edit.asp?id=<%=rs("id")%>" target="_parent"><strong style="color:#F00">编辑点评</strong></a>
  </li></UL>
    <% rs.movenext
			  else
				exit for
