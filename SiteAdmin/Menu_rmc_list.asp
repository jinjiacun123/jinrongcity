...
<%
	...
	set rs_news=server.CreateObject("ADODB.recordset")
		news_sql="select sq_jinrongcity.[LmTab_Rmc].*, sq_jinrongcity.[LmTab].* from sq_jinrongcity.[LmTab_Rmc] inner join sq_jinrongcity.[LmTab] on sq_jinrongcity.[LmTab_Rmc].NewsId=sq_jinrongcity.[LmTab].LmId where sq_jinrongcity.[LmTab_Rmc].FParentID=0 "

		if request.Form("butall")<>"" then
			response.Redirect("Menu_rmc_list.asp")
		end if	
		
		if request("butdel")<>"" then
	 for each item in request("delchoose")
	 ''''删除小图
	  sql="select Server_filepath from sq_jinrongcity.[LmTab_Rmc] where id="& item
		...
		if rs_news("Server_filepath")<>"" then
			sql="select Server_filepath from sq_jinrongcity.[LmTab_Rmc] where FParentID="& item
			if rs_news.state=1 then rs_news.close
			rs_news.open sql,conn,1,3
			...
			conn.execute "delete from sq_jinrongcity.[LmTab_Rmc] where id="& item & " or FParentID="& item
		else
			conn.execute "delete from sq_jinrongcity.[LmTab_Rmc] where id="& item
		end if	
...
function ShowRDlgRet(toLink,iW,iH)  //带返回值的框架对话框
 {  
	...
	function searchrecord(){
	  var bLogged;
	  bLogged=ShowRDlgRet('Menu_rmc_find.asp',500,150);
...
<body>
<form name="form1" method="post" action="Menu_rmc_list.asp">
        ...
        <td>
         <table width="100%"  border="0" cellpadding="4" cellspacing="1" bgcolor="#EBEBEB">
            <tr>
              ...
              <td width="7%" background="images/headerbg1.gif"><div align="center" class="headtext">热门词</div></td>
			  <td width="54%" background="images/headerbg1.gif"><div align="center" class="headtext">信息标题</div></td>
            ...
            <tr <%if i mod 2 = 1 then response.Write("bgcolor='#FFFFFF'") else response.Write("bgcolor='#FAFAFA'")%>>
              ...
              <td align="center"><span class="line150"><%=rs_news("LmName")%></span></td>
			  <td><span class="line150"><%=rs_news("title")%></span></td>
              <td><div align="left"><%=rs_news("author")%></div></td>
              <td><div align="center"><img src="images/del.gif" width="16" height="16" align="absmiddle"><a href="Menu_rmc_edit.asp?id=<%=rs_news("ID")%>">编辑</a></div></td>
            </tr>
			<% rs_news.movenext
	  else
        exit for
