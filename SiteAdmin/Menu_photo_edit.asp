...
'		rs.open "update Img_tab set Server_filename='' , Server_filepath='' , Client_filename='' where id=" & upload.form("id") & " or FParentID=" & upload.form("id") ,connstr,1,3
'		response.Redirect("Menu_photo_edit.asp?id="&upload.form("id"))
'	end if		

	if upload.form("butdel")<>"" then
	'''删除小图
	  sql="select Server_filepath, Server_filename, Client_filename,id from sq_jinrongcity.[Img_tab] where id="& upload.form("id")
		...
		conn.execute "update sq_jinrongcity.[Img_tab] set Server_filename='' , Server_filepath='' , Client_filename='' where id="& upload.form("id") & " or FParentID=" & upload.form("id")
		else
...
'if upload.form("butdel")<>"" then
'	 ''''删除小图
'	  sql="select Server_filepath from Img_tab where id="& upload.form("id")
'		if rs.state=1 then rs.close
'		rs.open sql,conn,1,3
'		
'		Oldfilename=rs("Server_filepath")
...
'		''''删除大图
'		if rs("Server_filepath")<>"" then
'			sql="select Server_filepath,FParentID from Img_tab where FParentID="& upload.form("id")
'			if rs.state=1 then rs.close
'			rs.open sql,conn,1,3
'			
'			if not rs.eof then
...
'				end if	
'			end if	
'			conn.execute "update Img_tab set Server_filename='' , Server_filepath='' , Client_filename='' where id="& upload.form("id") & " or FParentID=" & upload.form("id")
'		else
'			conn.execute "update Img_tab set Server_filename='' , Server_filepath='' , Client_filename='' where id="& upload.form("id")
'		end if	
'		
'		response.Redirect("Menu_photo_edit.asp?id="&upload.form("id"))	
'	end if
...
<TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td height="26" background="images/headerbg1.gif">&nbsp; <span class="headtext"><img src="images/dec_1.gif" width="11" height="11" align="absmiddle"> 编辑新闻</span></td>
      </tr>
      <tr>
...
<TABLE cellSpacing=1 cellPadding="4" width="100%" align="center" border=0>
			 ...
			 <% if rs("Server_filepath")<>"" then %>
			  ...
			  <TR>
			    <TD align="right"  width=106 bgColor="#E5EFF7"><font color="#FF0000">*</font> 所属栏目：</TD>
			    <TD colspan="3" align="left" bgColor="#F7F7F7"><select name="cmbLMid" id="cmbLMid">
				<option value="1" <% if rs("NewsId")="1" then %> selected="selected" <%end if%>>滚动图片</option>
				<option value="2" <% if rs("NewsId")="2" then %> selected="selected" <%end if%>>观点一区</option>
                <option value="3" <% if rs("NewsId")="3" then %> selected="selected" <%end if%>>资讯一区</option>
                <option value="4" <% if rs("NewsId")="4" then %> selected="selected" <%end if%>>普通文章</option>
				</select></TD>
			  </TR>	
			  <TR>
			    <TD align="right"  width=106 bgColor="#E5EFF7"><font color="#FF0000">*</font> 新闻标题：</TD>
			    <TD colspan="3" align="left" bgColor="#F7F7F7"><input type="text" name="txttitle" size="50" class="a2" value="<%=rs("title")%>"></TD>
			  </TR>			  		
				<TR>
			      <TD align="right" bgColor="#E5EFF7"><font color="#FF0000">*</font> 新闻摘要：</TD>
			      <TD colspan="3" align="left" bgColor="#F7F7F7"><input type="text" name="txtsource" size="50" class="a2" value="<%=rs("source")%>"></TD>
		        </TR>
                <TR>
			      <TD align="right" bgColor="#E5EFF7"><font color="#FF0000">*</font> 新闻作者：</TD>
