...
<%
	...
	set rs_news=server.CreateObject("ADODB.recordset")
		news_sql="select * from sq_jinrongcity.[Img_tab] where Img_tab.FParentID=0 "

		if request.Form("butall")<>"" then
			response.Redirect("Menu_photo_list.asp")
		end if	
		
	if request("butdel")<>"" then
	 for each item in request("delchoose")
	 ''''删除小图
	  sql="select Server_filepath from sq_jinrongcity.[Img_tab] where id="& item
		...
		''''删除大图
		if rs_news("Server_filepath")<>"" then
			sql="select Server_filepath from sq_jinrongcity.[Img_tab] where FParentID="& item
			if rs_news.state=1 then rs_news.close
			rs_news.open sql,conn,1,3
			...
			conn.execute "delete from sq_jinrongcity.[Img_tab] where id="& item & " or FParentID="& item
		else
			conn.execute "delete from sq_jinrongcity.[Img_tab] where id="& item
		end if	
...
<body>
<form name="form1" method="post" action="Menu_photo_list.asp">
  ...
  <tr>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#EBEBEB">
            <tr>
              ...
              <td width="560">&nbsp;</td>
              <td width="274">
                <div align="right"><input name="butdel" type="submit" class="buttomw" value="多重删除">&nbsp;&nbsp;<input name="butsearch" type="submit" class="buttomw" value="新闻搜索" title="新闻搜索" onClick="return searchrecord();"><input type="hidden" name="Tj">&nbsp;&nbsp;<input name="butall" type="submit" class="buttomw" value="刷 新">&nbsp;&nbsp;</div></td>
            </tr>
          </table>
        ...
        <td>
         <table width="100%"  border="0" cellpadding="4" cellspacing="1" bgcolor="#EBEBEB">
            <tr>
              ...
              <td width="11%" background="images/headerbg1.gif"><div align="center" class="headtext">录入时间</div></td>
              <td width="7%" background="images/headerbg1.gif"><div align="center" class="headtext">新闻栏目</div></td>
			  <td width="54%" background="images/headerbg1.gif"><div align="center" class="headtext">新闻标题</div></td>
			  <td width="9%" background="images/headerbg1.gif"><div align="center" class="headtext">录入者</div></td>
            ...
            <tr <%if i mod 2 = 1 then response.Write("bgcolor='#FFFFFF'") else response.Write("bgcolor='#FAFAFA'")%>>
              ...
              <td><div align="left"><%=rs_news("Indate")%></div></td>
              <td align="center"><span class="line150">
			  <%if rs_news("NewsId")=1 then%>图片滚动
			  <%elseif rs_news("NewsId")=2 then%>观点一区
			  <%elseif rs_news("NewsId")=3 then%>资讯一区
			  <%elseif rs_news("NewsId")=4 then%>普通文章
			  <%end if%></span></td>
			  <td><span class="line150"><%=rs_news("title")%></span></td>
              <td><div align="left"><%=rs_news("author")%></div></td>
              <td><div align="center"><img src="images/del.gif" width="16" height="16" align="absmiddle"><a href="Menu_photo_edit.asp?id=<%=rs_news("ID")%>">编辑</a></div></td>
            </tr>
			<% rs_news.movenext
	  else
        exit for
