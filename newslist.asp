...
<%
	...
	set rs=Server.CreateObject("ADODB.Recordset")
		...
		if typeid=1 then
			typestr="滚动图片"
			news_sql=" select * from sq_jinrongcity.[Img_tab] where FParentID=0 and NewsId=1 "
		
		elseif typeid=2 then
			typestr="观点"
			news_sql=" select * from sq_jinrongcity.[Img_tab] where FParentID=0 and NewsId=2 "
			
		elseif typeid=3 then
			typestr="资讯"
			news_sql=" select * from sq_jinrongcity.[Img_tab] where FParentID=0 and NewsId=3 "
		
		elseif typeid=4 then
			typestr="热点"
			news_sql=" select * from sq_jinrongcity.[Img_tab] where FParentID=0 and NewsId=4 "
		end if
...
<div class="main">
	...
	<div class="contentB1">
		...
		%>
				<li>
					<a href="newsdetail.asp?id=<%response.Write(rs("id"))%>" target="_blank">
